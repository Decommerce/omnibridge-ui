...
export const useBridgeDirection = () => {
    ...
    const bridgeConfig = useMemo(
        () => networks[bridgeDirection] || Object.values(networks)[0], [bridgeDirection],
    );
    ...
    const getBridgeChainId = useCallback(
        chainId => (chainId === homeChainId ? foreignChainId : homeChainId), [homeChainId, foreignChainId],
    );

    const getMonitorUrl = useCallback(
        (chainId, hash) => `${ambLiveMonitorPrefix}/${chainId}/${hash}`, [ambLiveMonitorPrefix],
    );
    ...
    const getGraphEndpoint = useCallback(
        chainId => {
            ...
            if (homeChainId === 8787 || homeChainId === 4) {
                return subgraphName
            }
    ...
    const getAMBAddress = useCallback(
        chainId => (chainId === homeChainId ? homeAmbAddress : foreignAmbAddress), [homeChainId, homeAmbAddress, foreignAmbAddress],
    );

    const getTotalConfirms = useCallback(
        chainId =>
        chainId === homeChainId ? homeTotalConfirms : foreignTotalConfirms, [homeChainId, homeTotalConfirms, foreignTotalConfirms],
    );
